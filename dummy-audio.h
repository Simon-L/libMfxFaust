            ...
            if (fNumInputs > 0) {
                for (int frame = 0; frame < fSample; frame++) {
                    for (int chan = 0; chan < fNumInputs; chan++) {
                        std::cout << std::fixed << std::setprecision(10) << "\t chan " << chan << " in, frame " << frame << " = " << fInChannel[0][frame];
                    }
                    std::cout << std::endl;
                }
            }
            if (fNumOutputs > 0) {
                for (int frame = 0; frame < fSample; frame++) {
                    for (int chan = 0; chan < fNumOutputs; chan++) {
                        std::cout << std::fixed << std::setprecision(10) << "\t chan " << chan << " out, frame " << frame << " = " << fOutChannel[chan][frame];
                    }
                    std::cout << std::endl;
                }
            }
